-- ============================================================================
-- 017 VERIFY — run the whole migration, prove it BEHAVES, throw it away.
-- Ends in `rollback;`. Safe against production by construction.
-- Prints "all 017 behaviour checks passed" or raises the assertion that failed.
-- ============================================================================
...
do $$
declare hh uuid; me uuid; kid uuid; t1 uuid; ev uuid; n int;
begin
  ...
  -- due_time drives the nag hour
  insert into todos (household_id, title, assignee_id, due_on, due_time, created_by)
  values (hh,'trash',kid,current_date,'07:00',me) returning id into t1;
  perform queue_todo_nags();
  select count(*) into n from reminders r
   where r.todo_id=t1 and (r.fire_at at time zone (select timezone from households limit 1))::time = '07:00';
  if n <> 1 then raise exception 'FAIL: nag did not use due_time (got % rows)', n; end if;

  -- a todo with no due_time still nags at 6pm
  insert into todos (household_id, title, assignee_id, due_on, created_by)
  values (hh,'no time thing',kid,current_date,me) returning id into t1;
  perform queue_todo_nags();
  select count(*) into n from reminders r
   where r.todo_id=t1 and (r.fire_at at time zone (select timezone from households limit 1))::time = '18:00';
  if n <> 1 then raise exception 'FAIL: default nag hour lost'; end if;

  -- due_time carries to the spawned successor
  insert into todos (household_id, title, assignee_id, due_on, due_time,
                     repeat_freq, repeat_interval, repeat_days, created_by)
  values (hh,'weekly trash',kid,current_date - 21,'07:00','weekly',1,array[2],me)
  returning id into t1;
  update todos set completed_at = now() where id = t1;
  select count(*) into n from todos where parent_id = t1 and due_time = '07:00';
  if n <> 1 then raise exception 'FAIL: due_time not carried to successor'; end if;

  -- ticking a one-off event drops its unsent reminders
  insert into events (household_id, title, event_date, all_day, created_by)
  values (hh,'probe event',current_date + 3,true,me) returning id into ev;
  insert into reminders (household_id, event_id, member_id, lead_minutes, fire_at)
  values (hh, ev, me, 0, now() + interval '2 days');
  update events set done_at = now() where id = ev;
  select count(*) into n from reminders where event_id = ev and sent_at is null;
  if n <> 0 then raise exception 'FAIL: done event kept % unsent reminders', n; end if;

  -- An occurrence of a SERIES can be done without touching the series. The
  -- one-off above is deliberately not reused: for a one-off, done_at means
  -- done on every date, because there is only one.
  insert into events (household_id, title, event_date, all_day,
                      repeat_freq, repeat_interval, repeat_days, created_by)
  ...
  if not occurrence_done(ev, current_date) then raise exception 'FAIL: occurrence_done false'; end if;
  if occurrence_done(ev, current_date + 1) then raise exception 'FAIL: done leaked to another date'; end if;
  if occurrence_done(ev, current_date + 7) then raise exception 'FAIL: done leaked to next week'; end if;

  raise notice 'ALL 017 CHECKS PASSED';
end $$;
select 'all 017 behaviour checks passed' as result;
rollback;
