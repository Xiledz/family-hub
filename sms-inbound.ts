...
const isQualifierOnly = name => QUALIFIERS.has(String(name || '').toLowerCase().replace(/\s+/g, ' ').trim());
...
async function addShopping(db: any, body: string, sender: any) {
  ...
  for (const it of p.items) {
    ...
    if (looksMerged(it.name) || isFormOnly(it.name) || isQualifierOnly(it.name)) continue;
