...
select 'recipes' t, count(*)::text n from recipes
union all select 'meal_plan', count(*)::text from meal_plan
union all select 'reminders subjects',
  (select pg_get_constraintdef(oid) from pg_constraint where conname='reminders_one_subject');
